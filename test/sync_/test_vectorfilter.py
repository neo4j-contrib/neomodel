...
@mark_sync_test
def test_vectorfilter_thresholding():
    """
    ...
    class someNodeThresh(StructuredNode):
        name = StringProperty()
        vector = ArrayProperty(
            base_property=FloatProperty(), vector_index=VectorIndex(2, "cosine")
        )

    db.install_labels(someNodeThresh)

    john = someNodeThresh(name="John", vector=[float(0.5), float(0.5)]).save()
    fred = someNodeThresh(name="Fred", vector=[float(1.0), float(0.0)]).save()
    ...
    result = vectorsearchFilterThreshold.all()
...
@mark_sync_test
def test_vectorfilter_invalid_threshold_type():
    """
