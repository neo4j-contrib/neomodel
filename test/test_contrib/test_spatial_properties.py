...
class TestSpacial(unittest.TestCase):

    def test_spatial_point_property(self):
        """
        ...
        with self.assertRaisesRegex(ValueError, r'Invalid CRS\(None\)'):
            PointProperty()

        with self.assertRaisesRegex(ValueError, r'Invalid CRS\(crs_isaak\)'):
            PointProperty(crs='crs_isaak')

        with self.assertRaisesRegex(TypeError, 'Invalid default value'):
            PointProperty(default=(0.0, 0.0), crs='cartesian')
    ...
    def test_inflate(self):
        ...
        values_from_db = [(CartesianPoint((0.0, 0.0)),
    ...
    @unittest.skip("Don't know what is wrong here. Needs expert to fix")
    def test_default_value(self):
        ...
        def get_some_point():
            return NeomodelPoint((random.random(), random.random()))
        ...
        class LocalisableEntity(neomodel.StructuredNode):
            ...
            location = PointProperty(crs='cartesian', default=get_some_point)
        ...
        assert retrieved_object.location == NeomodelPoint(coords), "Default value assignment failed."
    ...
    def test_array_of_points(self):
        ...
        class AnotherLocalisableEntity(neomodel.StructuredNode):
            ...
            """
            identifier = neomodel.UniqueIdProperty()
            locations = neomodel.ArrayProperty(PointProperty(crs='cartesian'))

        # Neo4j versions lower than 3.4.0 do not support Point. In that case, skip the test.
        check_and_skip_neo4j_least_version(340, 'This version does not support spatial data types.')

        an_object = AnotherLocalisableEntity(locations=[NeomodelPoint((0.0, 0.0)), NeomodelPoint((1.0, 0.0))]).save()
        ...
        assert retrieved_object.locations == [NeomodelPoint((0.0, 0.0)), NeomodelPoint((1.0, 0.0))], \
