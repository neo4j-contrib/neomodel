...
class PydanticPerson(PydanticBridge, AsyncStructuredNode):
...
def test_use_db_aliases():
    class Aliased(PydanticBridge, AsyncStructuredNode):
...
@mark_async_test
async def test_instance_round_trip_with_save():
    person = await PydanticPerson(name="Alice", role="admin", tags=["a", "b"]).save()
