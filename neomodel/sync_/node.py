...
_T = TypeVar("_T", bound="StructuredNode")
...
class _NodesProperty:
    ...
    ``NodeSet[MyNode]`` for type checkers, so ``MyNode.nodes.get(...)`` is
    known to return ``MyNode`` (and iteration yields ``MyNode``). At runtime it
    behaves like the previous classproperty.
    """

    def __get__(self, instance: Any, owner: type[_T]) -> NodeSet[_T]:
        from neomodel.sync_.match import NodeSet

        return NodeSet(owner)
...
class StructuredNode(NodeBase):
    ...
    def _bulk_create(cls, *props: tuple, **kwargs: dict[str, Any]) -> list:
        ...
        results, _ = db.cypher_query(query, {"create_params": create_params})
    ...
    @classmethod
    def bulk_save(cls, nodes: list[StructuredNode]) -> list[StructuredNode]:
        """
        ...
        if to_create:
            ...
            results, _ = db.cypher_query(create_query, {"create_params": create_params})
        ...
        if to_update:
            id_method = db.get_id_method()
            rows = [
                {
                    "eid": db.parse_element_id(node.element_id),
            ...
            db.cypher_query(update_query, {"rows": rows})
