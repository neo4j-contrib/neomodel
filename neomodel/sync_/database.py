"""
Database facade for the neomodel module.

``Database`` is the singleton ``db`` that user code and the rest of
neomodel talk to. It is a thin facade that composes three collaborators and
delegates to them:

* :class:`~neomodel.sync_.connection.ConnectionManager` - driver, URL,
  server version/edition, sessions and transactions (the shared state).
* :class:`~neomodel.sync_.query.QueryRunner` - Cypher execution, streaming
  and object resolution.
* :class:`~neomodel.sync_.schema.SchemaManager` - index/constraint
  installation and schema admin.

The node-class registry lives in :mod:`neomodel.sync_._registry`.
...
from neomodel.sync_.query import QueryRunner, _redact_params
from neomodel.sync_.schema import SchemaManager

# Re-exported for backward compatibility (imported from this module elsewhere).
__all__ = ["Database", "db", "ensure_connection", "_redact_params"]
...
class Database:
    ...
    It is a facade over an ``ConnectionManager`` (connection/transaction
    state), an ``QueryRunner`` (query execution) and an
    ...
    def __init__(self) -> None:
        ...
        self._query = QueryRunner(self._connection)
        self._connection._query = self._query
        self._schema = SchemaManager(self._connection, self._query)
    ...
    def set_connection(
        ...
        self._connection.set_connection(url=url, driver=driver)
    ...
    def database_version(self) -> str | None:
        return self._connection.database_version

    @property
    def database_edition(self) -> str | None:
        return self._connection.database_edition
    ...
    def begin(
        ...
        **parameters: Any,
        ...
        self._connection.begin(access_mode=access_mode, timeout=timeout, **parameters)
    ...
    def get_id_method(self) -> str:
        return self._connection.get_id_method()

    def parse_element_id(self, element_id: str | None) -> str | int:
        return self._connection.parse_element_id(element_id)

    def version_is_higher_than(self, version_tag: str) -> bool:
        return self._connection.version_is_higher_than(version_tag)

    def edition_is_enterprise(self) -> bool:
        return self._connection.edition_is_enterprise()

    def parallel_runtime_available(self) -> bool:
        return self._connection.parallel_runtime_available()
    ...
    def cypher_query(
        ...
        return self._query.cypher_query(
            query,
            params,
            handle_unique,
    ...
    def list_indexes(self, exclude_token_lookup: bool = False) -> list[dict]:
        return self._schema.list_indexes(exclude_token_lookup=exclude_token_lookup)

    def list_constraints(self) -> list[dict]:
        return self._schema.list_constraints()

    def change_neo4j_password(self, user: str, new_password: str) -> None:
        self._schema.change_neo4j_password(user, new_password)

    def clear_neo4j_database(
        self, clear_constraints: bool = False, clear_indexes: bool = False
    ) -> None:
        self._schema.clear_neo4j_database(
    ...
    def drop_constraints(
        ...
        self._schema.drop_constraints(quiet=quiet, stdout=stdout)

    def drop_indexes(self, quiet: bool = True, stdout: TextIO | None = None) -> None:
        self._schema.drop_indexes(quiet=quiet, stdout=stdout)

    def remove_all_labels(self, stdout: TextIO | None = None) -> None:
        self._schema.remove_all_labels(stdout=stdout)

    def install_all_labels(self, stdout: TextIO | None = None) -> None:
        self._schema.install_all_labels(stdout=stdout)

    def install_labels(
        self, cls: Any, quiet: bool = True, stdout: TextIO | None = None
    ) -> None:
        self._schema.install_labels(cls, quiet=quiet, stdout=stdout)
