"""
Query execution for the neomodel module.

``QueryRunner`` runs Cypher (in the active transaction or an auto-commit
...
``ConnectionManager`` held as ``self.db``.
"""
...
from neo4j import Query, Result, Session, Transaction
    ...
    def _run_cypher_query(
        ...
        try:
            ...
            if retry_on_session_expire:
                self.db.set_connection(url=self.db._connection_url)
