    ...
    def __init__(self, **kwargs):
        ...
        for name, property in properties:
            ...
            if getattr(property, 'choices', None):
                setattr(self, f'get_{name}_display',
        ...
        if base_property is not None:
            ...
            for ilegal_attr in ['default', 'index', 'unique_index', 'required']:
                if getattr(base_property, ilegal_attr, None):
                    raise ValueError(f'ArrayProperty base_property cannot have "{ilegal_attr}" set')
...
class DateTimeProperty(Property):
    ...
    def inflate(self, value):
        try:
            ...
            raise ValueError(f"Float or integer expected, got {type(value)} can't inflate to datetime.")
        except TypeError:
            raise TypeError(f"Float or integer expected. Can't inflate {type(value)} to datetime.")
...
class UniqueIdProperty(Property):
    ...
    def __init__(self, **kwargs):
        for item in ['required', 'unique_index', 'index', 'default']:
            if item in kwargs:
                raise ValueError(f'{item} argument ignored by {self.__class__.__name__}')
