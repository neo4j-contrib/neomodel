...
class BaseSet(Generic[T]):
    ...
    limit: int | None
    skip: int | None
    def __getitem__(self, key: int | slice) -> BaseSet[T] | None: ...
...
class NodeSet(BaseSet[T]):
    ...
    def has(self, **kwargs: Any) -> NodeSet[T]: ...
    def order_by(self, *props: Any) -> NodeSet[T]: ...
    def unique_variables(self, *paths: str) -> NodeSet[T]: ...
    def traverse(self, *paths: tuple[str, ...], **aliased_paths: dict) -> NodeSet[T]: ...
    def annotate(self, *vars: tuple, **aliased_vars: tuple) -> NodeSet[T]: ...
    def resolve_subgraph(self) -> list: ...
    def subquery(self, nodeset: NodeSet, return_set: list[str], initial_context: list[str] | None = None) -> NodeSet[T]: ...
    def intermediate_transform(self, vars: dict[str, Transformation], distinct: bool = False, ordering: list | None = None) -> NodeSet[T]: ...

class Traversal(BaseSet[T]):
    definition: dict
    source: Any
    source_class: Any
    target_class: type[T]
    ...
    def match(self, **kwargs: Any) -> Traversal[T]: ...
